...
app = FastAPI(title="FastAPI Day 2 Assignment", description="Advanced Product & Order Management")
...
class BulkOrder(BaseModel):
    ...
    contact_email: str = Field(..., min_length=5) # Can use EmailStr if pydantic[email] is installed
...
# --- Q3: Accept Customer Feedback (POST) ---
@app.post("/feedback")
...
# Standard order model for the bonus task
class OrderRequest(BaseModel):
    product_id: int
    quantity: int

@app.post("/orders")
def place_single_order(request: OrderRequest):
    new_order = {
        "order_id": len(orders) + 1,
        "product_id": request.product_id,
        "quantity": request.quantity,
        "status": "pending"  # Requirement: Start as pending
    }
...
def get_all_feedback():
    """Returns all submitted customer feedback."""
